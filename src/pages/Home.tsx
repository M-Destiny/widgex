  ...
  return (
    <div className="space-y-16">
      <section className="bg-gradient-to-br from-blue-600 to-purple-700 text-white py-24 px-6">
        <div className="max-w-4xl mx-auto text-center space-y-6">
          ...
          <div className="flex gap-4 justify-center">
            <Link to="/catalog" className="px-6 py-3 bg-white text-blue-600 rounded-lg font-semibold hover:bg-gray-100 transition-colors flex items-center gap-2">
              Browse Catalog <ArrowRight size={18} />
            </Link>
            <Link to="/dashboard" className="px-6 py-3 border border-white/30 text-white rounded-lg font-semibold hover:bg-white/10 transition-colors">
              My Dashboard
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 space-y-8">
        <h2 className="text-3xl font-bold">Featured Widgets</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {featured.map((w) => <WidgetCard key={w.id} widget={w} />)}
        </div>
        <div className="text-center">
          <Link to="/catalog" className="text-blue-600 font-medium hover:underline inline-flex items-center gap-1">
